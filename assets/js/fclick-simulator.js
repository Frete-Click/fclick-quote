    ...
    .then(function (response) {
        $.each(response.data, function (index, val) {
            if(val.success === false){
                addError('Não foi possível localizar seu cadastro, preencha os dados solicitados!');
            }else{
...
async function getQuotes(){
    ...
    if(checkInputs() === true){
        quoteData = {
            ...
            productTotalPrice: convert_price($('#product-invoice-total').val()),
            ...
            contact: 
            {
                email:  $('#custumer-email').val(),
                name:   $('#custumer-name').val(),
                phone:  $('#custumer-phone').val()
            }
        }
...
async function addCarrirToList(image, price, retrieveDeadline, deadline){
